...
class CompactCalendarController {
    ...
    private void loadAttributes(AttributeSet attrs, Context context) {
        if (attrs != null && context != null) {
            TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, com.github.sundeepk.compactcalendarview.R.styleable.CompactCalendarView, 0, 0);
            try {
                currentDayBackgroundColor = typedArray.getColor(R.color.white, currentDayBackgroundColor);
                calenderTextColor = typedArray.getColor(R.color.white, calenderTextColor);
                currentSelectedDayBackgroundColor = typedArray.getColor(R.color.white, currentSelectedDayBackgroundColor);
                calenderBackgroundColor = typedArray.getColor(R.color.white, calenderBackgroundColor);
                textSize = typedArray.getDimensionPixelSize(R.color.white,
    ...
    void drawMonth(Canvas canvas, Calendar currentMonthToDrawCalender, int offset) {
        ...
        boolean isSameYear = currentMonthToDrawCalender.get(Calendar.YEAR) == todayCalender.get(Calendar.YEAR);
        int todayDayOfMonth = todayCalender.get(Calendar.DAY_OF_MONTH);
        ...
        for (int dayColumn = 0, dayRow = 0; dayColumn <= 6; dayRow++) {
            ...
            if (dayRow == 0) {
                ...
                if (isSameYear&&isSameMonth && todayDayOfMonth == day) {
    ...
    private void drawCircle(Canvas canvas, float x, float y, int color) {
        ...
        float radius = (float) (3*Math.sqrt(widthPerDay * widthPerDay + heightPerDay * heightPerDay));
