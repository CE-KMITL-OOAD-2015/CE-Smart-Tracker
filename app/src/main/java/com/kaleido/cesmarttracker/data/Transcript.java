...
public class Transcript {
    ...
    public HashMap<Course,Double> getTakenCoursesByCategory(String category) {
        HashMap<Course, Double> takenCourses = new HashMap<>();
        ArrayList<Course> courses = new ArrayList<>(getAllTakenCourses().keySet());
        for (int i = 0; i < courses.size(); i++) {
            if (courses.get(i).getCategory().contains(category)) {
                takenCourses.put(courses.get(i), getAllTakenCourses().get(courses.get(i)));
            }
        }
        return takenCourses;
    }
